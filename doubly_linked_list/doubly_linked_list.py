...
class DoublyLinkedList:
    ...
    def add_to_tail(self, value):
        ...
        # otherwise
            ...
            # set the new node next to the current tail
    ...
    def delete(self, node):
        # decrement the length
        self.length -= 1
        # if not head or tail then return
        if not self.head and not self.tail:
            return
        ...
        # otherwise if tail is the node
            ...
            # set the tail to the nodes previous
            self.tail = node.prev
            # delete node
            node.delete()
        # otherwise
    ...
    def get_max(self):
        # if no head then return None
        if not self.head:
            return None
        # set the initial max value to the head value
